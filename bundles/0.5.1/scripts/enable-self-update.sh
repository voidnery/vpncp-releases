...
if [ -z "$VERSION" ]; then
  VERSION="$(curl -fsSL --max-time 20 "$BASE/$CHANNEL.json" | grep -o '"version":"[^"]*"' | head -1 | cut -d'"' -f4 || true)"
fi
...
while read -r path sha; do
  ...
  curl -fsSL --max-time 60 "$BASE/bundles/$VERSION/$path" -o "$TMP/$path" 2>/dev/null \
    || curl -fsSL --max-time 60 "$BASE/bundle/$path" -o "$TMP/$path" \
