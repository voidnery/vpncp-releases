...
echo "3b. Host networking (IPv4 vs IPv6)"
# A host whose IPv6 route is blackholed makes plain curl hang while containers,
# which sit on Docker's IPv4-only network, work fine. This asymmetry is confusing
# to diagnose from the panel, so name it explicitly.
BASE_CHK="${VPNCP_RELEASES_BASE_URL:-https://raw.githubusercontent.com/voidnery/vpncp-releases/main}"
_v4="$(curl -4 -sS -m 10 -o /dev/null -w '%{http_code}' "$BASE_CHK/stable.json" 2>/dev/null || echo 000)"
_v6="$(curl -6 -sS -m 10 -o /dev/null -w '%{http_code}' "$BASE_CHK/stable.json" 2>/dev/null || echo 000)"
[ "$_v4" = "200" ] && ok "host reaches the release repo over IPv4" || bad "host cannot reach the release repo over IPv4 ($_v4)"
if [ "$_v6" != "200" ] && [ "$_v4" = "200" ]; then
  info "IPv6 fails ($_v6) while IPv4 works — scripts must force -4 (they do)"
fi
echo
