#!/usr/bin/env bash
# =============================================================================
# Enables in-panel self-update on an EXISTING install — run once, on the host.
#
#   ./scripts/enable-self-update.sh            # interactive (shows a diff first)
#   ./scripts/enable-self-update.sh --yes      # no prompt (for automation)
#
# Why this exists at all, and why it cannot be done from the panel:
#   Enabling self-update means granting a container access to the Docker socket,
#   i.e. root on this host. Nothing already running inside Docker may hand itself
#   that privilege — if the panel could, a compromised panel could root the host.
#   So exactly one deliberate act by someone with host access is required. After
#   this, the updater keeps the deploy files in sync with every release on its
#   own; this script is never needed again.
#
# What it does: downloads the deploy files for the release you run, verifies each
# against the sha256 in that release's manifest, backs up what you have, shows a
# diff, and recreates the stack. Idempotent — safe to re-run.
# =============================================================================
set -euo pipefail

FILE="${VPNCP_COMPOSE_FILE:-docker-compose.dist.yml}"
BASE="${VPNCP_RELEASES_BASE_URL:-https://raw.githubusercontent.com/voidnery/vpncp-releases/main}"
ASSUME_YES=0
[ "${1:-}" = "--yes" ] && ASSUME_YES=1

c_blue()  { printf '\033[1;34m%s\033[0m\n' "$1"; }
c_green() { printf '\033[1;32m%s\033[0m\n' "$1"; }
c_red()   { printf '\033[1;31m%s\033[0m\n' "$1"; }
c_yell()  { printf '\033[1;33m%s\033[0m\n' "$1"; }

need() { command -v "$1" >/dev/null 2>&1 || { c_red "missing required tool: $1"; exit 1; }; }
need curl; need sha256sum; need docker

# fetch <url> <dest-file>
#
# Two transports, in order:
#   1. curl from the host, forced to IPv4. A host with a broken/blackholed IPv6
#      route (seen in the wild: AAAA resolves, nothing answers) makes plain curl
#      hang until timeout even though IPv4 works perfectly.
#   2. wget from inside the api container. Docker's default network is IPv4-only,
#      so containers keep working when the host's own routing is broken. This is
#      not a theoretical fallback — it is exactly the state one live install was
#      in when this script kept failing.
fetch() {
  curl -4 -fsSL --max-time 30 "$1" -o "$2" 2>/dev/null && [ -s "$2" ] && return 0
  docker compose -f "$FILE" exec -T api sh -c "wget -q -O- --timeout=30 '$1'" > "$2" 2>/dev/null \
    && [ -s "$2" ] && { c_yell "(fetched via the api container — host networking failed)"; return 0; }
  return 1
}

[ -f "$FILE" ] || { c_red "$FILE not found — run this from your install folder (e.g. /opt/vpncp)."; exit 1; }

c_blue "=== Enable in-panel self-update ==="
echo

# --- 1. Which release's deploy files should we install? ----------------------
# The LATEST release on the channel, not the version this install happens to be
# running. Deploy files are forward-compatible, and an install that is a release
# or two behind would otherwise try to verify against a manifest whose files are
# no longer the ones published. Override with: VPNCP_TARGET_VERSION=0.4.9 ./…
CHANNEL="${VPNCP_CHANNEL:-stable}"
VERSION="${VPNCP_TARGET_VERSION:-}"
if [ -z "$VERSION" ]; then
  if fetch "$BASE/$CHANNEL.json" /tmp/.vpncp-channel.json; then
    VERSION="$(grep -o '"version":"[^"]*"' /tmp/.vpncp-channel.json | head -1 | cut -d'"' -f4 || true)"
    rm -f /tmp/.vpncp-channel.json
  fi
fi
RUNNING="$(docker compose -f "$FILE" exec -T api sh -c 'echo "$APP_VERSION"' 2>/dev/null | tr -cd 'A-Za-z0-9._-' || true)"
[ -n "$RUNNING" ] && [ "$RUNNING" != "$VERSION" ] && \
  c_yell "This install runs $RUNNING; installing deploy files from the latest $CHANNEL release ($VERSION)."
if [ -z "$VERSION" ]; then
  c_red "Could not determine a release version."
  c_yell "Neither the host nor the api container could reach $BASE."
  c_yell "Check: curl -4 -sS -m 10 -o /dev/null -w '%{http_code}\n' $BASE/$CHANNEL.json"
  exit 1
fi
c_blue "Target release: $VERSION"

fetch "$BASE/versions/$VERSION.json" /tmp/.vpncp-manifest.json || {
  c_red "No manifest for $VERSION at $BASE/versions/$VERSION.json"; exit 1; }
MANIFEST="$(cat /tmp/.vpncp-manifest.json)"; rm -f /tmp/.vpncp-manifest.json

# --- 2. Fetch + verify each deploy file --------------------------------------
TMP="$(mktemp -d)"; trap 'rm -rf "$TMP"' EXIT
PAIRS="$(printf '%s' "$MANIFEST" | tr -d ' \n' \
  | grep -o '"path":"[^"]*","sha256":"[0-9a-f]*"' \
  | sed -e 's/"path":"//' -e 's/","sha256":"/ /' -e 's/"$//')"

if [ -z "$PAIRS" ]; then
  c_red "Release $VERSION predates deploy-file sync (no 'deploy' section in its manifest)."
  c_yell "Cut a newer release first, then re-run this script."
  exit 1
fi

CHANGED=0
while read -r path sha; do
  [ -n "$path" ] || continue
  case "$path" in
    docker-compose.dist.yml|ops/updater/watch.sh) ;;
    *) c_yell "skipping unexpected path in manifest: $path"; continue ;;
  esac
  mkdir -p "$TMP/$(dirname "$path")"
  # Immutable per-version path first; bundle/ is overwritten by every release and
  # only matches the newest manifest.
  fetch "$BASE/bundles/$VERSION/$path" "$TMP/$path" \
    || fetch "$BASE/bundle/$path" "$TMP/$path" \
    || { c_red "download failed: $path"; exit 1; }
  got="$(sha256sum "$TMP/$path" | cut -d' ' -f1)"
  if [ "$got" != "$sha" ]; then
    c_red "checksum mismatch for $path"
    c_red "  expected (manifest $VERSION): $sha"
    c_red "  downloaded:                   $got"
    c_yell "The published files no longer match this release's manifest. Usually this means"
    c_yell "a newer release exists — re-run without VPNCP_TARGET_VERSION to take the latest."
    exit 1
  fi
  if [ -f "$path" ] && cmp -s "$TMP/$path" "$path"; then
    echo "  = $path (already current)"
  else
    echo "  + $path (will be updated)"
    CHANGED=1
  fi
done <<EOF
$PAIRS
EOF

if [ "$CHANGED" = "0" ]; then
  c_green "Deploy files are already up to date."
else
  echo
  c_blue "--- changes to be applied ---"
  while read -r path sha; do
    [ -n "$path" ] || continue
    [ -f "$path" ] && diff -u "$path" "$TMP/$path" || true
  done <<EOF
$PAIRS
EOF
  echo
  c_yell "Review the diff above. Local edits (ports, domains, extra services) will be LOST"
  c_yell "unless you re-apply them afterwards. Backups are written as <file>.bak."
  if [ "$ASSUME_YES" = "0" ]; then
    printf 'Apply and recreate the stack? [y/N] '
    read -r answer
    case "$answer" in y|Y|yes|YES) ;; *) c_yell "Aborted. Nothing was changed."; exit 0 ;; esac
  fi
  while read -r path sha; do
    [ -n "$path" ] || continue
    mkdir -p "$(dirname "$path")"
    [ -f "$path" ] && cp -f "$path" "$path.bak"
    cp -f "$TMP/$path" "$path"
  done <<EOF
$PAIRS
EOF
  c_green "Deploy files updated (backups: *.bak)."
fi

# --- 2b. Pin the project directory ------------------------------------------
# The updater must see this folder at its real host path; otherwise compose
# resolves relative bind mounts (./ops/nginx/conf.d) against the wrong root and
# silently mounts empty directories, which takes nginx down on every update.
HERE="$(pwd)"
if grep -q '^VPNCP_COMPOSE_DIR=' .env 2>/dev/null; then
  cur="$(grep '^VPNCP_COMPOSE_DIR=' .env | head -1 | cut -d= -f2-)"
  if [ "$cur" != "$HERE" ]; then
    sed -i "s|^VPNCP_COMPOSE_DIR=.*|VPNCP_COMPOSE_DIR=$HERE|" .env
    c_green "Updated VPNCP_COMPOSE_DIR to $HERE"
  fi
elif [ -f .env ]; then
  printf '\n# Absolute path of this install; the updater mounts it at the same path.\nVPNCP_COMPOSE_DIR=%s\n' "$HERE" >> .env
  c_green "Added VPNCP_COMPOSE_DIR=$HERE to .env"
else
  c_red ".env not found — create it before enabling self-update."; exit 1
fi

# --- 3. Recreate so the api picks up VPNCP_UPDATE_COMMAND --------------------
c_blue ""
c_blue "Recreating the stack…"
docker compose -f "$FILE" up -d

# --- 4. Verify the thing we actually care about ------------------------------
sleep 3
CMD="$(docker compose -f "$FILE" exec -T api sh -c 'echo "$VPNCP_UPDATE_COMMAND"' 2>/dev/null | tr -d '\r' || true)"
echo
if [ -n "$CMD" ]; then
  c_green "Self-update is ENABLED (api sees: $CMD)"
  c_green "The panel now shows 'Обновить сейчас' whenever a newer release exists."
  c_green "From here on, deploy files update themselves with each release."
else
  c_red "VPNCP_UPDATE_COMMAND is still empty in the api container."
  c_yell "Try: docker compose -f $FILE up -d --force-recreate api"
  c_yell "Then re-run ./scripts/check-self-update.sh"
  exit 1
fi
